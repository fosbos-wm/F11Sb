...
<strong>${t[1]}</strong><small>${t[2]}</small>${!t[4]?`<span class="badge" style="margin-top:8px">IN VORBEREITUNG</span>`:""}</a>`;
  return `${pageHead("ZUSAMMENARBEIT","Tools für Zusammenarbeit",
    "Kostenlose, direkt in die F11Sb integrierte Tools für Gruppenarbeit, Brainstorming und Unterricht – ganz ohne externe Anmeldung.",
...
async function downloadWordcloudPDF(wordcloudId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Wortwolke"+(cloud.description?" · "+cloud.description:"")
    );
...
async function downloadKanbanPDF(boardId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Kanban-Board"+(board.description?" · "+board.description:"")
    );
...
async function downloadTermPollPDF(pollId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Terminfindung · "+votes.length+" Stimme(n) insgesamt"+(poll.description?" · "+poll.description:"")
    );
...
async function downloadChecklistPDF(checklistId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Gemeinsame Checkliste · "+done+" von "+items.length+" erledigt"+(list.description?" · "+list.description:"")
    );
...
async function downloadAmpelPDF(roundId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Verständnis-Ampel · "+total+" Antwort(en)"+(round.description?" · "+round.description:"")
    );
...
async function downloadPollPDF(pollId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Live-Umfrage · "+total+" Stimme(n)"+(poll.description?" · "+poll.description:"")
    );
...
async function downloadDeckPDF(deckId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Karteikarten · "+cards.length+" Karte(n)"+(deck.description?" · "+deck.description:"")
    );
...
async function downloadEssayPDF(caseId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Fachaufsatz-Training"+(c.theoryArea?" · "+c.theoryArea:"")
    );
...
async function renderPinnwandUebersicht(){
  ...
  return `${pageHead("ZUSAMMENARBEIT","Pinnwand",
    "Digitale Pinnwände für Ideen, Brainstorming und Gruppenarbeit – im Raster, für die ganze F11Sb sichtbar.",
...
async function downloadBoardPDF(boardId){
  try{
    ...
    openToolPrintWindow(
      ...
      body,
      "F11Sb · Pinnwand"+(board.description?" · "+board.description:"")
    );
...
function buildICS(events,calName){
  const lines=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//F11Sb//Kalender//DE","CALSCALE:GREGORIAN",
    `X-WR-CALNAME:${escapeICS(calName||"F11Sb Kalender")}`];
...
async function downloadLernstandResultPDF(id){
  try{
    ...
    openToolPrintWindow(
      ...
      "F11Sb · Lernstandsmessung "+t.nr+"/26 · "+(LERNSTAND_AREAS[t.learningArea]?.title||"")
    );
